...
- occurrence local-sync identity and snapshot diff matching are intentionally more stable than source fingerprints: they reconcile the same class/date/period occurrence across source-fingerprint drift and small metadata edits so preview can emit exact/update work instead of recreating every unchanged lesson
