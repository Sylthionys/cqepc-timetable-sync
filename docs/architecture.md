...
- smoke coverage also captures the scrolled Settings calendar-display section in both light and dark themes so theme repaint regressions are visible in automation artifacts
...
Preview generation remains local-first, but the Google preview path now uses two windows:
...
Within that flow, exact same-title same-time Google events suppress duplicate Adds and stay visible on Home as neutral existing items, while same-title different-time Google events become red delete candidates only when they fall inside the deletion window.
...
Google apply now executes accepted calendar changes in delete -> update -> add order so switching the selected class clears stale managed events before creating replacement events for the new class slice.
