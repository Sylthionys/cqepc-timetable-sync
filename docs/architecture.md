...
- occurrence local-sync identity and snapshot diff matching are intentionally more stable than source fingerprints: the local snapshot diff first deduplicates comparable previous occurrences, then pairs old/new occurrences by exact identity or by same class/date plus source, title, time/period, location, teacher, and profile evidence. This reconciles source-fingerprint drift and small metadata edits as exact/update work instead of recreating every unchanged lesson, while still leaving true removals as deletes.
