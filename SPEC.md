...
- Home preview can optionally import the selected Google calendar into the board so existing remote events are visible before apply, with added timetable items in green, app-managed delete candidates in red with strikethrough, unrelated or unmanaged Google items kept on a neutral current-calendar surface, and exact same-time managed matches kept on a neutral existing-item surface
...
- After a Google apply, if preview still shows app-managed duplicate events for the same class, title, and time range and the current normalized timetable only needs one of them, the convergence pass must automatically apply the represented remote instance delete change for each extra duplicate, re-read Google, and continue cleanup instead of leaving two identical lessons visible. Location text must not prevent this cleanup because stale Google duplicates can differ only by parsed/defaulted location metadata.
...
- same title + different time inside the semester window: show the remote item as a red delete candidate only when it is app-managed; unmanaged Google events remain neutral awareness-only context and must not become delete actions
- remote items outside the semester delete window: show as neutral awareness-only context, but do not create delete actions
...
Deleted occurrence details should not expose the generic edit form first. Their right-panel action is Cancel Delete; it creates a one-time local schedule override marked as a retained deleted occurrence. That retained single-occurrence override is the narrow exception to active-binding filtering: it may survive when the current parse no longer contains that source occurrence, so the preview can cancel the pending delete before apply. A retained flag without `SourceOccurrenceDate`, an ordinary override, or an override from an old source file, another selected class, or any other inactive source context must not be regenerated.
...
For Google Calendar, accepted calendar writes must execute in deterministic delete -> update -> add order so class switches and drift repairs remove stale managed events before creating replacement events, and calendar writes should run serially for reliability when large batches include overlapping recurring changes. Managed Google color is part of the update payload, so a selected change that only differs by `colorId` must still repair the existing managed event instead of being treated as unchanged or recreated. The apply path should preserve preview-resolved managed duplicate recurring-instance deletes as separate instance writes, while still reusing recurring-instance lookups per series for mapped update/delete repairs so large batch repairs do not degrade into repeated list-instance scans. When multiple managed remote events resolve to the same course occurrence, preview must keep only the expected exact-match count and leave the extra managed copies represented as deletions rather than suppressing every duplicate as exact. If Google returns an incomplete recurring-series materialization after insert, the provider must roll that series back and fall back to exact single-event writes instead of leaving the user with a partially applied schedule.
