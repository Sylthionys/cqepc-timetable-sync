...
Deleted occurrence details should not expose the generic edit form first. Their right-panel action is Cancel Delete; it creates a one-time local schedule override marked as a retained deleted occurrence, so the next preview can treat it as retained/unchanged even though the parsed source no longer contains that binding, and then allow normal detail editing.
...
- Program Settings
  ...
  - Custom proxy supports optional username/password, `Bypass local`, and a bypass list that defaults to `localhost`, `127.0.0.1`, and `::1` so Google OAuth loopback callbacks never go through the custom proxy; proxy passwords are stored outside JSON using user-local DPAPI protection
