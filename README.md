...
- Custom proxy passwords are stored outside JSON in `%LocalAppData%\CQEPC Timetable Sync\network-proxy-password.bin` using user-local DPAPI protection. The network proxy setting affects only remote provider clients such as Google OAuth/Calendar/Tasks and Microsoft Graph; local PDF/XLS/DOCX parsing and local storage remain direct local operations.
