...
- Home preview merges parsed timetable occurrences with Google Calendar remote-event recognition, keeps added timetable items green, app-managed delete candidates red with strikethrough, leaves unrelated or unmanaged Google items as neutral current-calendar context, filters unmanaged Google update/delete/metadata-only rows out of selectable/apply sets, and only suppresses exact matches for Google items that are already app-managed for the same occurrence identity
...
- after a Google apply, the Home convergence pass now detects app-managed duplicate calendar events for the same class/title/time even when their Google location text differs, automatically applies the represented remote-instance delete rows for the extras, and re-reads Google between cleanup batches so recurring duplicates are removed one concrete instance at a time until only the expected occurrence count remains
...
- provider-safe Google event metadata storage via `extendedProperties.private` plus a local Google sync-mapping store for remote IDs and source fingerprints; ordinary Google Calendar descriptions are treated as display text and never establish app-managed ownership or destructive sync identity
...
- Home render caching stores prepared display rows by preview/selection/calendar-display mode and, when explicitly enabled, can persist the last rendered Home board as a DPAPI-protected startup display hint while the normal preview refresh replaces stale cached content
...
- The last Home render cache is off by default. When the user enables it, the app stores the startup display hint under `%LocalAppData%\CQEPC Timetable Sync\home-schedule-render-cache.bin` using user-local DPAPI protection, deletes any legacy plaintext `home-schedule-render-cache.json`, and still treats the normal local/provider preview refresh as authoritative.
...
- Google Calendar notes now render as a single-column code-style red/green line diff. Added/deleted rows omit note diff blocks, note-only updates show a notes update instead of expanding every parsed metadata segment as its own field change, and unusually large legacy descriptions keep a bounded diff by summarizing the changed middle block with a few context lines.
